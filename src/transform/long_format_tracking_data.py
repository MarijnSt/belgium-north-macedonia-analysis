...
def transform_tracking_to_long_format(tracking_df: pd.DataFrame) -> pd.DataFrame:
    ...
    try:
        ...
        pivoted = melted.pivot_table(
            index=['frame_id', 'period_id', 'timestamp', 'wall_clock', 'team_id', 'shirt_number'],
            columns='property',
            values='value',
            aggfunc='first'
        ).reset_index()
        ...
        ball_cols = [col for col in id_vars if col not in ['ball_status', 'last_touch', 'ball_z']]
        ...
        return result[['frame_id', 'period_id', 'timestamp', 'wall_clock', 
                    'team_name', 'shirt_number', 'x', 'y', 'speed']]
